...
const Header: React.FC = () => {
  ...
  // logger
  console.log(navbarOpen, "oo");

  return (
    <header
      className={`fixed top-0 z-40 w-full pb-5 transition-all duration-300 ${
        sticky
          ? "shadow-lg bg-darkmode/60 backdrop-blur-sm pt-5" // 90% opacity when sticky + blur
          : "shadow-none  backdrop-blur-sm md:pt-14 pt-5" // 60% opacity when not sticky + blur
      }`}
    >
      <div className="lg:py-0 py-2">
        <div className="container mx-auto lg:max-w-screen-xl md:max-w-screen-md flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* <Link
              href="#main-banner"
              className="hidden md:flex text-darkmode bg-primary border border-primary py-2 px-4 rounded-lg sm:text-21 text-18 font-medium hover:bg-transparent hover:text-primary"
            >
              Download App
            </Link> */}
            {isSignUpOpen && (
        ...
        <div
          ref={mobileMenuRef}
          className={`lg:hidden fixed top-0 right-0 h-screen w-full bg-darkmode shadow-lg transform transition-transform duration-300 max-w-xs ${
        ...
        >
          ...
          <nav className="flex flex-col items-start p-4 bg-[#112546ff]">
            {headerData.map((item, index) => (
              <MobileHeaderLink
                key={index}
                item={item}
